...
export default function TalentNode({ talent }: TalentNodeProps) {
  ...
  const spellId = talent.spell_tooltip?.spell?.id;

  return (
    <a 
      href={`https://www.wowhead.com/classic/spell=${spellId}`}
      data-wowhead={`spell=${spellId}&domain=classic`}
      className={`relative w-12 h-12 border-2 ${borderClass} rounded bg-zinc-950 flex items-center justify-center cursor-pointer hover:border-gold transition-colors`}
    >
      {iconUrl ? (
