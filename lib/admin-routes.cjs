...
function createAdminRouter() {
  ...
  // Detecta se deve usar a pasta externa (fora de nodejs) ou interna
  const externalUploads = path.resolve(__dirname, "..", "..", "uploads");
  const internalUploads = path.resolve(__dirname, "..", "data", "uploads");
  const uploadDir = fs.existsSync(externalUploads) ? externalUploads : internalUploads;
  ...
  router.post("/change-password", async function (req, res) {
    ...
    try {
      ...
      const match = auth.comparePassword(oldPassword, user.password);
      if (!match) {
  ...
  // PERSISTÊNCIA SIMPLES DE ERROS EM MEMÓRIA
  const errorLogs = [];

  router.post("/logs", async function (req, res) {
    const { message, stack } = req.body || {};
    errorLogs.unshift({ timestamp: new Date().toISOString(), message, stack });
  ...
  router.put("/users/:id", auth.requireRole(["admin"]), async function (req, res) {
    try {
      ...
      if (password) updateData.password = auth.hashPassword(password);
  ...
  router.post("/users/:id/reset-password", auth.requireRole(["admin"]), async function (req, res) {
    try {
      ...
      const hashedPassword = auth.hashPassword(password);
      const ok = await db.updateUserPassword(req.params.id, hashedPassword);
