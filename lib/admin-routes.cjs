...
function createAdminRouter() {
  ...
  router.post("/register", async function (req, res) {
    ...
    if (secret !== guildSecret) return res.status(401).json({ error: "Palavra-passe incorreta" });
    try {
  ...
  router.put("/adventures/:id", auth.requireRole(["admin", "guildmaster", "officer"]), async function (req, res) {
    try {
      const item = await db.updateAdventure(req.params.id, req.body);
  ...
  router.get("/users", auth.requireRole(["admin"]), async function (req, res) {
    try {
      ...
      res.json({ users: list.map(u => ({ id: u.id, username: u.username, role: u.role })) });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
