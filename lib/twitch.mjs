export const TAG = "<BRAZUG>";

/** Jogos WoW no Twitch (retail + classic) */
export const WOW_GAME_IDS = ["18122", "1122165802", "493057"];

let cachedToken = null;
let tokenExpiresAt = 0;

export function titleHasBrazugTag(title) {
...
export async function getAppAccessToken() {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      "Configure TWITCH_CLIENT_ID e TWITCH_CLIENT_SECRET (variáveis de ambiente ou .env)"
    );
  }
...
async function fetchStreamsPage(gameId, cursor, token, clientId) {
  ...
  if (!res.ok) {
    throw new Error("Twitch API streams (" + res.status + ")");
  }

  return res.json();
}

export async function collectBrazugStreams() {
  const { token, clientId } = await getAppAccessToken();
  const byLogin = new Map();
  const maxPagesPerGame = 8;
...
export function liveStreamsPayload(streams) {
