...
function createCharacterRouter() {
  ...
  router.get("/armory/items", async (req, res) => {
    try {
      const { q, limit, slot } = req.query;
      if (!q) return res.json([]);
      
      let slotTypes = null;
      if (slot) {
        const slotMapping = {
          'HEAD': ['HEAD'], 'NECK': ['NECK'], 'SHOULDER': ['SHOULDER'], 'BACK': ['CLOAK'],
          'CHEST': ['CHEST', 'ROBE'], 'SHIRT': ['SHIRT'], 'TABARD': ['TABARD'], 'WRIST': ['WRIST'],
          'HANDS': ['HANDS'], 'WAIST': ['WAIST'], 'LEGS': ['LEGS'], 'FEET': ['FEET'],
          'FINGER_1': ['FINGER'], 'FINGER_2': ['FINGER'], 'TRINKET_1': ['TRINKET'], 'TRINKET_2': ['TRINKET'],
          'MAIN_HAND': ['MAIN_HAND', 'ONE_HAND', 'TWO_HAND', 'WEAPON'],
          'OFF_HAND': ['OFF_HAND', 'ONE_HAND', 'WEAPON', 'SHIELD', 'HOLDABLE'],
          'RANGED': ['RANGED', 'RANGED_RIGHT', 'THROWN', 'RELIC']
        };
        slotTypes = slotMapping[slot] || null;
      }
      
      const results = await db.searchArmoryItems(q, parseInt(limit) || 20, slotTypes);
      res.json(results);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.post("/character/sync", async (req, res) => {
    ...
    try {
      console.log(`[SYNC-PROXY] Forcing sync for ${name}-${realm} at ${ARMORY_URL}`);
      ...
      const data = await response.json().catch(() => ({ error: "Resposta inválida do módulo Armory" }));

      if (!response.ok) {
        console.error(`[SYNC-PROXY-ERROR] Armory failed with status ${response.status}:`, data);
        return res.status(response.status).json(data);
      }

      res.json(data);
    } catch (err) {
      console.error(`[SYNC-PROXY-CRITICAL] Failed to connect to Armory: ${err.message}`);
      res.status(500).json({ 
        error: "Falha crítica na conexão com o módulo de sincronização.", 
        details: err.message
      });
    }
  });
