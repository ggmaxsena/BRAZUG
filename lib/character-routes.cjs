...
function createCharacterRouter() {
  ...
  const ARMORY_URL = process.env.ARMORY_URL || "http://127.0.0.1:3001";
  ...
  router.post("/character/sync", async (req, res) => {
    ...
    try {
      console.log(`[SYNC-PROXY] Proxying sync for ${name}-${realm} to ${ARMORY_URL}`);
      ...
      console.error(`[SYNC-PROXY-ERROR] Failed to proxy sync:`, err.message);
      res.status(500).json({ error: err.message });
    }
  });
