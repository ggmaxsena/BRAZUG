...
function createCharacterRouter() {
  ...
  router.get("/armory/items/:id", async (req, res) => {
    try {
      ...
      // Se o item não tem tooltip_data, tenta forçar um fetch via módulo Armory
      if (!item || !item.tooltip_data) {
        console.log(`[Proxy] Fetching missing item data for ${itemId} from Armory...`);
        try {
          const armoryRes = await fetch(`${ARMORY_URL}/api/item/${itemId}`);
          if (armoryRes.ok) {
            // O Armory salvou no banco compartilhado, vamos ler de novo
