...
// MONGODB_URI_STANDARD (mongodb://...) = hosts explícitos, sem lookup SRV.
const MONGO_URI_RAW = process.env.MONGODB_URI_STANDARD
...
async function resolveConnectionUri() {
  ...
  console.log(
    "[BRAZUG] mongodb+srv → connection string Standard (hosts de shard)"
  );
  return resolvedMongoUri;
}
