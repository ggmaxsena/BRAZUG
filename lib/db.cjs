...
// Polyfill para ambientes onde crypto global pode estar ausente (necessário para ObjectId do Mongo)
if (typeof global !== "undefined" && !global.crypto) {
  try {
    const cryptoModule = require("crypto");
    // No Node 18+, o webcrypto é a implementação recomendada
...
function mapMongoDoc(doc) {
  ...
  // Mapeamento explícito para evitar problemas de serialização e circularidade
  return {
...
async function listAdventures(publishedOnly = true, visibility = "public") {
  ...
  // Excluir image_data da listagem para manter o payload leve
...
async function getAdventureImage(id) {
  ...
  const doc = await collection.findOne(query, { projection: { image_data: 1 } });
...
async function updateAdventure(id, data) {
  ...
  // Compatibilidade com diferentes versões do driver Mongo
