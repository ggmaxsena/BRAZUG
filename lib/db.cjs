...
async function updateAdventure(id, data, authenticatedUser = null) {
  ...
  if (authenticatedUser.role !== "admin") {
    const adv = await client.query("SELECT user_id FROM adventures WHERE id = $1::uuid", [id]);
    if (!adv.rows[0]) return null;

    const user = await getUserByUsername(authenticatedUser.username);
    if (!user || adv.rows[0].user_id !== user.id) {
      return null;
    }
  }

  const res = await client.query(
    `UPDATE adventures SET title=$2, body=$3, author=$4, image_url=$5, image_data=$6, event_date=$7::date, published=$8, visibility=$9, updated_at=NOW()
     WHERE id=$1::uuid RETURNING *`,
    [id, data.title, data.body, data.author, data.image_url, data.image_data, data.event_date, data.published, data.visibility]
  );
  return mapAdventureRow(res.rows[0]);
}
...
async function deleteAdventure(id, authenticatedUser = null) {
  ...
  const res = await client.query("DELETE FROM adventures WHERE id = $1::uuid", [id]);
...
async function updateCharacter(id, data, authenticatedUser = null) {
    ...
    if (authenticatedUser.role !== "admin") {
      const char = await client.query("SELECT user_id FROM wow_characters WHERE id = $1::uuid", [id]);
      if (!char.rows[0]) return null;

      const user = await getUserByUsername(authenticatedUser.username);
      
      console.log(`[DEBUG-AUTH] Update Char: DB owner=${char.rows[0].user_id}, UserID=${user?.id}`);

      if (!user || String(char.rows[0].user_id) !== String(user.id)) {
        return null; // Não é o dono
      }
    }

    console.log(`[DEBUG-UPDATE] Char ID: ${id}, is_dead: ${data.is_dead}, Data:`, JSON.stringify(data));
    const res = await client.query(
        `UPDATE wow_characters SET name=$2, class=$3, race=$4, level=$5, guild=$6, image_url=$7, lore=$8, visibility=$9, is_dead=$10, updated_at=NOW()
         WHERE id=$1::uuid RETURNING *`,
        [id, data.name, data.class, data.race, data.level, data.guild, data.image_url, data.lore, data.visibility, data.is_dead]
    );
    return mapCharacterRow(res.rows[0]);
}
...
async function deleteCharacter(id, authenticatedUser = null) {
    ...
    if (authenticatedUser.role !== "admin") {
      const char = await client.query("SELECT user_id FROM wow_characters WHERE id = $1::uuid", [id]);
      if (!char.rows[0]) return false;

      const user = await getUserByUsername(authenticatedUser.username);
      if (!user || char.rows[0].user_id !== user.id) {
        return false;
      }
    }

    const res = await client.query("DELETE FROM wow_characters WHERE id = $1::uuid", [id]);
    return res.rowCount > 0;
}
  ...
  try {
    ...
    try { 
      const r = await client.query(`
        SELECT ci.*, i.name, i.quality, i.icon_filename, i.item_level, i.stats_text, i.description 
    ...
    return {
        ...
        professions,
        profile,
        // Fallback para campos que o frontend pode esperar
        avatarUrl: char.avatarUrl || (profile ? profile.image_url : null)
    };
  } catch (err) {
...
async function createSale(data) {
    ...
    const client = await getPool();
    const res = await client.query(
        `INSERT INTO sales (character_id, item_name, price, description)
         VALUES ($1, $2, $3, $4) RETURNING *`,
        [data.character_id, data.item_name, data.price, data.description]
    );
    return res.rows[0];
}
