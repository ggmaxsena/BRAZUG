...
function extractVideoData(url) {
    ...
    if (twitchClipMatch) return { platform: 'twitch', id: twitchClipMatch[1] };
...
async function acceptSale(saleId, bidderCharacterId, authenticatedUser) {
    ...
    const res = await client.query(`
        UPDATE sales 
        SET status = 'accepted', buyer_character_id = $2 
        WHERE id = $1::uuid 
...
async function purchaseSale(saleId, buyerCharacterId, authenticatedUser) {
    ...
    const char = await client.query("SELECT id FROM wow_characters WHERE id = $1 AND user_id = $2", [buyerCharacterId, authenticatedUser.id]);
    if (char.rowCount === 0 && authenticatedUser.role !== 'admin') throw new Error("Personagem não autorizado");

    // Marca a venda como 'sold' e registra o comprador
    const res = await client.query(`
        UPDATE sales 
        SET status = 'sold', buyer_character_id = $2 
...
module.exports = {
  ...
  getFullArmoryCharacter, listSales, createSale, listSalesByCharacter, deleteSale, acceptSale, purchaseSale,
