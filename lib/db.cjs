...
function matchId(itemId, searchId) {
...
async function getMongoCollection() {
  if (!mongoUri || mongoFailed) return null;
  ...
  try {
    ...
    console.error("[BRAZUG] MongoDB connection failed, falling back to JSON:", err.message);
    mongoFailed = true;
    return null;
  }
}
...
async function getAdventure(id) {
  ...
  const rows = readDb();
  return rows.find((item) => matchId(item.id, id)) || null;
}
...
async function updateAdventure(id, data) {
  ...
  const rows = readDb();
  const index = rows.findIndex((item) => matchId(item.id, id));
...
async function deleteAdventure(id) {
  ...
  const rows = readDb();
  const filtered = rows.filter((item) => !matchId(item.id, id));
