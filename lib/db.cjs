...
async function setResetToken(email, token, expires) {
  ...
  const client = await getPool();
  const result = await client.query(
    "UPDATE users SET reset_token = $2, reset_expires = $3 WHERE email = $1 RETURNING *",
    [email, token, expires]
  );
  return mapUserRow(result.rows[0]);
}
...
async function resetPassword(token, hashedPassword) {
  ...
  const client = await getPool();
  const result = await client.query(
    "UPDATE users SET password = $2, reset_token = NULL, reset_expires = NULL WHERE reset_token = $1 RETURNING *",
...
async function createUser(data) {
  ...
  const vToken = data.verification_token || null;
  ...
  try {
    const result = await client.query(
      `INSERT INTO users (username, email, password, role, verification_token)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [payload.username, email, payload.password, payload.role, vToken]
    );
    return mapUserRow(result.rows[0]);
  } catch (err) {
    if (err.code === "23505") {
      if (err.detail && err.detail.includes("email")) throw new Error("E-mail já cadastrado");
