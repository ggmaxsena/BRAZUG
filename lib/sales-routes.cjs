...
function createSalesRouter() {
  ...
  router.get("/items/search", async function (req, res) {
    try {
      const { name, quality, category } = req.query;
      const client = await db.getPool();
      
      let query = "SELECT id, name, quality, slot FROM item WHERE 1=1";
      const params = [];
      let i = 1;
      ...
      if (quality) {
        query += ` AND LOWER(quality) = LOWER($${i++})`;
        params.push(quality);
      }
      if (category) {
        query += ` AND LOWER(slot) = LOWER($${i++})`;
  ...
  router.post("/", async function (req, res) {
    try {
      // Basic validation
      const { character_id, item_name, item_id, price, description, duration_hours } = req.body;
      if (!character_id || !item_name || !price || !duration_hours) {
        return res.status(400).json({ error: "Dados incompletos" });
      }

      const sale = await db.createSale({ character_id, item_name, item_id, price, description, duration_hours }, req.user);
  ...
  router.post("/:saleId/bids", async function (req, res) {
    try {
      ...
      const bid = await db.createBid({ sale_id: saleId, bidder_character_id, amount }, req.user);
