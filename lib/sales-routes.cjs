...
function createSalesRouter() {
  ...
  router.get("/items/search", async function (req, res) {
    try {
      ...
      // Get items
      const queryItems = `SELECT id, name, quality, slot, icon_filename, max_stack ${queryBase} ORDER BY name ASC LIMIT ${limit} OFFSET ${offset}`;
  ...
  // LIST HISTORY
  router.get("/history/:characterId", async function (req, res) {
    try {
      const history = await db.listSalesHistory(req.params.characterId);
      res.json(history);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.post("/feedback", async function (req, res) {
    try {
      const feedback = await db.createFeedback(req.body, req.user);
  ...
  // GET REPUTATION
  router.get("/reputation/:characterId", async function (req, res) {
    try {
      const rep = await db.getCharacterReputation(req.params.characterId);
      const feedbacks = await db.listFeedbacksForCharacter(req.params.characterId);
      res.json({ ...rep, feedbacks });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
