...
async function collectBrazugStreams() {
  ...
  // ESTRATÉGIA DE BUSCA:
  // 1. Scan profundo em Português (PT) - Cobre 99% da guilda de forma ultra eficiente
  // 2. Scan raso Global - Para amigos/membros que usam outras linguagens
  
  const scanConfigurations = [
    { language: "pt", pages: 3 }, // Scaneia até 300 streams em PT (cobre quase todo WoW PT)
    { language: null, pages: 5 }  // Scaneia as top 500 globais
  ];
